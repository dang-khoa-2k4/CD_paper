...
The planner takes as input the collision-free convex cover
$\mathcal{Q}=\{Q_v\}$ constructed by either ACD or VCC. Each region is
represented by a vertex in a directed graph $G=(V,E)$; an edge $(u,v)$ denotes
a feasible transition between two adjacent regions. Source and target vertices,
denoted by $\sigma$ and $\tau$, encode the prescribed configurations
$\mathbf{x}_{\mathrm{start}}$ and $\mathbf{x}_{\mathrm{goal}}$, respectively.
Thus, the discrete part of the problem is to select a $\sigma$--$\tau$ path,
while the continuous part determines the trajectory inside the regions on that
path.

For a region $Q_v\subset\mathbb{R}^{n_c}$, the local trajectory is an
degree-$d$ B\'ezier curve
\begin{equation}
  \mathbf{q}_v(s)
...
$\Delta h_{v,k}=h_{v,k+1}-h_{v,k}$, $\varepsilon>0$ prevents a degenerate time
scaling, and $D$ is a convex velocity set. For each selected edge
$(u,v)$, position and time are joined through
\begin{equation}
...
endpoint in $Q_u\cap Q_v$.
Higher-order continuity constraints can be included in the edge model when a
particular application requires them.
...
$\mathcal{X}_e$ collect the compatibility constraints for edge $e=(u,v)$. GCS
...
The first constraint selects a $\sigma$--$\tau$ path, while the second enforces
state-flow consistency across adjacent local trajectories. The source and
target trajectory endpoints are fixed to the prescribed boundary configurations.
Depending on the chosen convex costs and sets, this model is a mixed-integer
convex program or a mixed-integer second-order cone program
~\cite{MarcucciEtAl2023GCS,MarcucciEtAl2023MotionPlanning}. It can combine
convex penalties on path length, traversal time, and trajectory smoothness.
